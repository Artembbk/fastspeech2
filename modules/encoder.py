...
class Encoder(nn.Module):
    ...
    def get_non_pad_mask(self, seq):
        assert seq.dim() == 2
        return seq.ne(self.model_config.PAD).type(torch.float).unsqueeze(-1)

    def get_attn_key_pad_mask(self, seq_k, seq_q):
        ''' For masking out the padding part of key sequence. '''
        # Expand to fit the shape of key query attention matrix.
        len_q = seq_q.size(1)
        padding_mask = seq_k.eq(self.model_config.PAD)
    ...
    def forward(self, src_seq, src_pos, return_attns=False):
        ...
        # -- Prepare masks
        slf_attn_mask = self.get_attn_key_pad_mask(seq_k=src_seq, seq_q=src_seq)
        non_pad_mask = self.get_non_pad_mask(src_seq)
